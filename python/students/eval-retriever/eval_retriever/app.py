"""Application module for eval-retriever — STUDENT STUB."""

import csv
from pathlib import Path

import mlflow  # noqa: F401  (you will need this once you implement _log_to_mlflow)
...
class App:
    ...
    def _log_to_mlflow(
        ...
        TODO (student exercise): implement MLFlow tracking for this run.

        Parameters
        ----------
        metrics : dict[str, float]
            Aggregated metrics from ``Query.aggregate``.
        n_samples : int
            Total number of samples evaluated.
        n_failures : int
            Samples that failed due to retriever errors.
        results_csv : Path
        ...
        Notes
        -----
        Step 1 — tell MLFlow where the server is::

            mlflow.set_tracking_uri(task_inputs.mlflow_tracking_uri)
            mlflow.set_experiment(task_inputs.experiment_name)

        Step 2 — open a run (use a ``with`` block)::

            with mlflow.start_run(run_name=task_inputs.approach_tag):
                ...

        Step 3 — inside the run, log four things:

        - A tag: ``mlflow.set_tag("approach", task_inputs.approach_tag)``
        - Params via ``mlflow.log_params({...})``:
          retriever_url, top_k, similarity_threshold, eval_csv_path,
          n_samples, n_failures
        - Metrics via ``mlflow.log_metrics(metrics)``
        - Artifact via ``mlflow.log_artifact(str(results_csv))``
        """
        raise NotImplementedError(
            "TODO: implement MLFlow logging — see docstring above for the steps."
        )
